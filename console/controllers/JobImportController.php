...
class JobImportController extends Controller
{
    ...
    {
        $jobList = JobInfoModel::find()->where(['isValid'=>JobInfoModel::IS_VALID_OK])->asArray()->all();
        if(count($jobList) > 0){
            $phone = array_unique(array_column($jobList,'phone'));
            User::updateAll(
                ['isValid'=>User::IS_VALID_NO],
                [
                    'and',
                    ['isValid'=>User::IS_VALID_OK],
                    ['!=','jobId',0],
                    ['not in','userAccount',$phone]
                ]
            );
        }
    }

    //处理数据  将删除的人才的user表信息删除
    public function actionHandle()
    {
        $delJob = JobInfoModel::find()->where(['isValid'=>JobInfoModel::IS_VALID_NO])->asArray()->all();
        $delJobIds = array_column($delJob,'jobId');
        if(count($delJobIds) > 0){
            foreach ($delJobIds as $delJobId) {
                $user = User::find()->where(['jobId'=>$delJobId])->one();
                if(!empty($user)){
    ...
    {
        //查找需要执行定时任务的数据
        $dataImport = DataImportLogModel::find()->where([
            'status'=>DataImportLogModel::IMPORT_ING,
            'type'=>DataImportLogModel::TYPE_JOB,
            'isConsole'=>DataImportLogModel::IS_CONSOLE_YES,
            'isValid'=>DataImportLogModel::IS_VALID_OK
        ])->asArray()->all();
        if(count($dataImport) <= 0){
            return false;
        }
        foreach ($dataImport as $item) {
            $this->importData($item['id']);
        }
        return true;
    }
...
//        ])->one();
        ...
        //设置 有需要导入的excel  正在执行
        $fileUrl = $dataImport->fileUrl;
        $staffId = $dataImport->staffId;
        $dataImportId = $dataImport->id;

        //先更改 这条导入记录已经执行了定时任务
        $dataImport->isConsole = DataImportLogModel::IS_CONSOLE_COMPLETED;
        if( !$dataImport->save() ){
            \Yii::info('------import data console fail update isConsole fail dataImportId--'.$dataImportId,'importJob');
            return false;
        }
        ...
        try{
            ...
            $dataImport->error = $exception->getCode() === Code::PARAM_ERR ? $exception->getMessage() : '数据处理失败';
            $dataImport->status = DataImportLogModel::COMPLETED_NO;
            $dataImport->save();
            return false;
        } finally {
        ...
        try {
            \Yii::info('-----import data---'.Json::encode($data),'importJob');
            \Yii::info('-----import data count---'.count($data),'importJob');
        ...
        try {
            ...
            \Yii::info('----import data count ----'.count($data));
