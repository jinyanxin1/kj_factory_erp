...
class ReturnGoodsDetailModel extends BaseModel
{
    ...
    {
        return 'jxt_jxc_return_goods_detail';
    }
     ...
     * 保存数据
     * */
    public static function saveData($data,$returnId,$houseId)
    {
        ...
        if(count($data) > 0){
            ...
            if(count($detailIds) > 0){//编辑时，一条进货记录中，如果某个商品在这条进货记录中删掉，那么，之前这个商品的进货记录也需要删掉，库存需要更新
                ...
                self::updateAll(['isValid'=>self::IS_VALID_NO],['returnDetailId'=>$detailIds]);
            }
