...
class ClientPersonModel extends BaseModel
{
	public static function tableName()
	{
		return 'kj_client_person';
	}
	...
	public static function getById($id, $select = ['*'], $asArray = false, $isIndex = '')
	{
		$model = self::find()
						->select($select)
						->Where([ 'personId' => $id , 'isValid' => self::IS_VALID_OK ]) ;
		if ($asArray) {
			$model = $model->asArray() ;
		}
		if ( !empty($isIndex) ) {
	...
	{
		self::updateAll(['isValid' => self::IS_VALID_NO ],[ 'personId' => $ids ]) ;
	}
}