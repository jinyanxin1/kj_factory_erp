...
class BorrowModel extends BaseModel
{
    ...
    public static function delById($id)
    {
        ...
        try{
            ...
            //删除领用，领用详情表
            BorrowModel::updateAll(['isValid'=>self::IS_VALID_NO,'updateTime'=>date('Y-m-d H:i:s')],['borrowId'=>$id]);
            BorrowDetailModel::updateAll(['isValid'=>BorrowDetailModel::IS_VALID_NO,'updateTime'=>date('Y-m-d H:i:s')],['borrowId'=>$id]);
            //删除库存记录
            if($type === self::TYPE_COLLAR_USE){
                //领用
               StockRecordModel::updateAll(['isValid'=>StockRecordModel::IS_VALID_NO,'updateTime'=>date('Y-m-d H:i:s')],['type'=>StockRecordModel::TYPE_COLLAR_USE,'unionId'=>$id]);
            }else if($type === self::TYPE_WITHDRAWAL_OF_COLLAR){
                //退领
                StockRecordModel::updateAll(['isValid'=>StockRecordModel::IS_VALID_NO,'updateTime'=>date('Y-m-d H:i:s')],['type'=>StockRecordModel::TYPE_WITHDRAWAL_OF_COLLAR,'unionId'=>$id]);
            }else{
                throw  new \Exception('领用类型错误');
            }
            $transaction->commit();
     ...
     * */
    public function saveData($borrowId,$data,$houseId,$remark,$userName,$date,$borrowName,$type,$goodsType)
    {
        ...
        try{
            ...
            $borrowInfo->goodsType = $goodsType;
            ...
            //保存详情
            $saveDetail = BorrowDetailModel::saveData($houseId,$data,$borrowId,$type,$userName,$remark,$date,$goodsType);
