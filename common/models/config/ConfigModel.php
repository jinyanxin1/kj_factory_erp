...
class ConfigModel extends BaseModel
{
	...
	public static function getById($id, $select = ['*'], $asArray = false, $isIndex = '')
	{
		$model = self::find()
						->select($select)
						->Where([ 'configId' => $id , 'isValid' => self::IS_VALID_OK ]) ;
		if ($asArray) {
			$model = $model->asArray() ;
		}
		if ( !empty($isIndex) ) {
	...
	public static function getPage($page = 0, $pageSize = 0, $select = ['*'], $andWhere = [], 
												$asArray = false, $orderBy = 'createTime desc', $indexBy = '', $groupBy = '')
	{
		$model = self::find()
						->select($select)
	...
	{
		self::updateAll(['isValid' => self::IS_VALID_NO,'updateTime'=>date('Y-m-d H:i:s') ],[ 'configId' => $ids ]) ;
	}
}