...
class WxPayApi
{
	...
	public static function unifiedOrder($inputObj, $timeOut = 6)
	{
		...
		Yii::warning('get-order-xia-dan-value(下单参数)'.print_r($result,true),'pay');
	...
	public static function refund($inputObj, $timeOut = 6)
	{
		...
		$result = WxPayResults::Init($response);
		Yii::info('get-return-pay-data'.print_r($result,true));
		self::reportCostTime($url, $startTimeStamp, $result);//上报请求花费时间
    ...
    /**
     *
     * 申请打款，WxPayRefund中out_trade_no、transaction_id至少填一个且
     * out_refund_no、total_fee、refund_fee、op_user_id为必填参数
     * appid、mchid、spbill_create_ip、nonce_str不需要填入
     * @param WxPayRefund $inputObj
     * @param int $timeOut
     ...
     * @return 成功时返回，其他抛异常
     */
    public static function cashOut($inputObj, $timeOut = 6)
    {
	...
	public static function notify($callback, &$msg)
	{
		...
		\Yii::warning('small-program-pay-call-back(获取微信支付结果再通知内容的xml)'.$xml,'pay');
		//如果返回成功则验证签名
		try {
		    ...
		    if($result['return_code'] == 'SUCCESS'){
		        $stationInfo=StationConfigModel::find()->where(['smallAppId'=>$result['appid']])->asArray()->one();
		        \Yii::warning("pay-success-call_back-start-data-stationInfo:" . json_encode($stationInfo),'pay');
		        //初始化参数
		        ...
		        $resultData = WxPayResults::Init($xml);
		        $result=$resultData['params'];
		    }
		    
		    
			\Yii::warning('small-program-pay-call-back(小程序支付结果再通知)data-'.print_r($result,true),'pay');
		} catch (WxPayException $e){
			$msg = $e->errorMessage();
			\Yii::warning('small-program-pay-call-back(获取微信支付结果再通知信息失败)','pay');
			return false;
		}
	 ...
	 * @throws WxPayException
	 */
    public static function postStr($url,$postfields){
        $ch = curl_init();
