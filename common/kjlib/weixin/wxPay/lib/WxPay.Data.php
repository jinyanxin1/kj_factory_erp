...
class WxPayResults extends WxPayDataBase
{
	...
	public function CheckSign()
	{
		//fix异常
		if(!$this->IsSignSet()){
		    \Yii::warning('small-program-pay-call-back(验证签名错误2)','pay');
		...
		\Yii::warning('small-program-pay-call-back(验证签名错误3)','pay');
		throw new WxPayException("签名错误！");
	}
	...
	{	
		...
		if($obj->values['return_code'] != 'SUCCESS'){
		    \Yii::warning('small-program-pay-call-back(验证签名错误1)','pay');
			 return array('return_code'=>'FAIL','params'=>$obj->GetValues());
		}
		// todo 验证签名
		$obj->CheckSign();
        return array('code'=>0,'params'=>$obj->GetValues());
	}
}
 ...
 *
 */
class WxPayCashOut extends WxPayDataBase
{
    /**
     * 设置微信分配的公众账号ID
     * @param string $value
