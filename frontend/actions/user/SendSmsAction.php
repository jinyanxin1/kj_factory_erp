...
class SendSmsAction extends Action
{
    ...
    public function run()
    {
        ...
        if(!preg_match("/^1[34578]\d{9}$/", $tel)){
            return $this->returnApi(100,'手机号格式不正确');
        }


        //短信每个手机号，每分钟只能发一次
        $redisKey = 'send:sms:tel:'.$tel.SmsLog::TYPE_ONE;
        $sendTime = Yii::$app->cache->get($redisKey);
        if(!empty($sendTime)){
            if((time() - strtotime($sendTime)) < 120){
                return $this->returnApi(100,'短信发送过于频繁');
            }
        }
        ...
        if( intval($sendSms['code']) === 0 ){
            //保存发短信记录
            SmsLog::saveData($code,$tel,$tel,SmsLog::TYPE_ONE);
