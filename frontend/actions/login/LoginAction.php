...
class LoginAction extends WxAction
{
...
//        $verifyCode = $this->request()->post('verifyCode', '');
        ...
        if ($userInfo->userType == 0) {
            return $this->returnApi(Code::NORMAL_ERR, '等待后台审核用户才能登陆');
        }
...
//        $loginModel->verifyCode = $verifyCode;
        ...
        */
        ...
        if ($loginModel->login()) {
            ...
            if (Yii::$app->user->identity->userType != UserModel::STAFF) {
                return $this->returnApi(Code::PARAM_ERR, '请使用职工账号登陆', []);
            }
